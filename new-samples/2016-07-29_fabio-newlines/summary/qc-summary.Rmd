...
When we did the pairwise comparisons, it made it hard to answer questions
about what the effect of treating with both Bortezomib and E7107 is, without
resorting to a crappy solution of doing Venn diagrams of the DE genes.

To get at that question, we'll look at the data slightly differently,
instead of comparing the treated back to the control samples, we instead are
going to look for effects that are specific to the treatment with Bortezomib or
E7107 alone and also look for genes that are specifically turned on when both
are treated. To do that we will re-analyze the data, but fit a model with an
interaction term, and then test for the effects of treatment with Bort alone,
treatment with E7107 alone and treamtent with both Bort and E7107.

```{r interactions}
summarydata$bort = ifelse(summarydata$treatment == "Bort", "yes",
                          ifelse(summarydata$treatment == "BE", "yes", "no"))
summarydata$E7107 = ifelse(summarydata$treatment == "E7107", "yes",
                    ifelse(summarydata$treatment == "BE", "yes", "no"))
...
These samples are all very very similar to each other within a sample class,
you can see they are almost perfectly correlated using Pearson correlation:
...
## Effect tables
[Bort effect](bort-effect.csv)

[E7107 effect](E7107-effect.csv)

[Combined effect](combined-effect.csv)

I talked with Praveen and it sounded like these were closer to being technical
variants than biological variants in terms of the experimental setup, and the
extremely high degree of correlation supports that. If that is true, it might
be worthwhile to just look at one sample of each and pretend like the
experiment was run with no replicates. We're in kind of a tough spot because
treating technical replicates as biological replicates is wrong, and leads to
what we are seeing where there are very high confidence changes when there
is only a small difference. If we do it the other way with no replicates,
then we lose a lot of power. However if these are actually technical replicates
than the 'power' we have is illusory since it isn't based on anything real.

# Effect reversal

Here we select genes which are significantly different from the control in the
combinination treatment and look how they correlate with the individual treatments:

## All genes affected by the combination of two drugs
```{r reversals}
sig = rownames(subset(be, padj < 0.05 & !is.na(padj) & abs(log2FoldChange) > 1 & baseMean > 50))
...
foldchanges = data.frame(bort=bort[sig,]$log2FoldChange,
                         e7107=e7107[sig,]$log2FoldChange,
                         both=be[sig,]$log2FoldChange)
ggplot(foldchanges, aes(bort, both)) + geom_point() +
  ylab("logFC treatment with bortezomib and E7107") +
  xlab("logFC treatment with bortezomib")
ggplot(foldchanges, aes(e7107, both)) + geom_point() +
  ylab("logFC treatment with bortezomib and E7107") +
  xlab("logFC treatment with E7107")
```

There are `r sum(bortrev & !e7107rev)` genes affected by the combination treatment
that have a combined affect in the opposite direction of the bortezomib alone effect.
There are `r sum(e7107rev & !bortrev)` genes affected by the combination treatment
that have a combined effect in the opposite direction of the E7107 alone effect.
And there are `r sum(bortrev & e7107rev)` genes affected by the combination treatment
that have a combined effect in the opposite direction than both the E7107
and bortezomib alone treatments.

## All genes affected in a non-additive way by the two drugs
Below we select the starting set of genes differently, by looking at the
genes with a significant interaction term. These are genes that appear to
have a non-additive effect of combining the two treatments:

```{r reversal-selected-interaction}
sig = rownames(subset(combinedeff, padj < 0.05 & !is.na(padj) & abs(log2FoldChange) > 1 & baseMean > 50))
bortrev = (bort[sig,]$log2FoldChange * be[sig,]$log2FoldChange) < 0
names(bortrev) = sig
e7107rev = (e7107[sig,]$log2FoldChange * be[sig,]$log2FoldChange) < 0
names(e7107rev) = sig
...
There are `r sum(bortrev & !e7107rev)` genes with a non-additive combination
treatment effect that have a combined affect in the opposite direction of the
bortezomib alone effect. There are `r sum(e7107rev & !bortrev)` genes with a
non-additive combination treatment effect that have a combined effect in the
opposite direction of the E7107 alone effect. And there are `r sum(bortrev &
e7107rev)` genes with a non-additive treatment effect that have a combined
effect in the opposite direction than both the E7107 and bortezomib alone
treatments.

## Amplified and dampened genes
Here we looked at all genes that were affected in a non-additive way
by the combination of the two drugs. Then we looked at genes that were
moving in the same direction in each of the single treatments and the
combination treatment, but were more strongly affected in the combination treatment
than expected by adding the two treatments together (amplified). Similarly
we looked for genes that were less strongly affected than expected by adding
the two treatments together (dampened).

```{r amplified-genes}
amplified = (e7107[sig,]$log2FoldChange > 0 &
             be[sig,]$log2FoldChange > 0 &
             bort[sig,]$log2FoldChange > 0 &
             combinedeff[sig,]$log2FoldChange > 0) |
  (e7107[sig,]$log2FoldChange < 0 &
   be[sig,]$log2FoldChange < 0 &
   bort[sig,]$log2FoldChange < 0 &
   combinedeff[sig,]$log2FoldChange < 0)
dampened = (e7107[sig,]$log2FoldChange > 0 &
             be[sig,]$log2FoldChange > 0 &
             bort[sig,]$log2FoldChange > 0 &
             combinedeff[sig,]$log2FoldChange < 0) |
  (e7107[sig,]$log2FoldChange < 0 &
   ...
   bort[sig,]$log2FoldChange < 0 &
   combinedeff[sig,]$log2FoldChange > 0)
reversed = bortrev & e7107rev
fcplot = data.frame(e7107=e7107[sig,]$log2FoldChange,
                    bort=bort[sig,]$log2FoldChange,
                    theoretical=e7107[sig,]$log2FoldChange + bort[sig,]$log2FoldChange,
                    actual=be[sig,]$log2FoldChange,
                    bortrev=bortrev,
                    e7107rev=e7107rev,
                    amplified=amplified,
                    dampened=dampened,
                    reversed=reversed,
                    effect=ifelse(amplified, "amplified",
                              ifelse(dampened, "dampened",
                              ifelse(reversed, "reversed",
                              ifelse(bortrev, "bortrev",
                              ifelse(e7107rev, "e7107rev",
                                     "none"))))))
fcplot$interactionFDR=combinedeff[sig,]$padj
fcplot$baseMean=combinedeff[sig,]$baseMean
rownames(fcplot) = sig
ggplot(fcplot, aes(theoretical, actual, color=effect)) + geom_point() +
  xlab("theoretical additive effect of both drugs") +
  ylab("actual effect when both drugs are applied") +
  labs(color="")
fcplot = fcplot %>% tibble::rownames_to_column() %>%
  left_join(symbols, by=c("rowname"="ens_gene")) %>%
  arrange(interactionFDR)
readr::write_csv(fcplot, "theoretical-vs-actual.csv")
```

[Theoretical vs actual](theoretical-vs-actual.csv)

# GEO output
For the GEO submission, they want to include only the control and the E7107
samples. We'll subset down the TPM samples and the counts, and then output
only the transcript/gene counts for the control and E7107 samples.

```
gene-TPM-E7017.csv
gene-counts-E7107.csv
transcript-TPM-E7107.csv
transcript-counts-E7107.csv
control-vs-E7107.csv
control-vs-E7107-sleuth.csv
```

```{r geo-output-gene-tpm}
keep = startsWith(summarydata$Name, "Ctrl") | startsWith(summarydata$Name, "E7107")
keep = summarydata$Name[keep]
sailfish_gene_tpm = readr::read_tsv("../combined.gene.sf.tpm")
sailfish_gene_tpm = sailfish_gene_tpm[, c("gene_id", keep)]
readr::write_csv(sailfish_gene_tpm, "geo/gene-TPM-E7107.csv")
```

```{r geo-output-transcript-tpm}
sailfish_transcript_tpm = readr::read_tsv("../combined.isoform.sf.tpm")
sailfish_transcript_tpm = sailfish_transcript_tpm[, c("id", keep)]
readr::write_csv(sailfish_transcript_tpm, "geo/transcript-TPM-E7107.csv")
```

```{r geo-output-gene-counts}
genecounts = counts(dds, normalized=FALSE)[, keep] %>%
  as.data.frame() %>%
  tibble::rownames_to_column("ensembl_gene_id")
readr::write_csv(genecounts, "geo/gene-counts-E7107.csv")
```

```{r geo-output-transcript-counts}
txi.salmon.tx = tximport(sf_files, type="salmon",
                         countsFromAbundance="lengthScaledTPM",
                         txOut=TRUE)
txcounts = round(txi.salmon.tx$counts)[, keep] %>%
  as.data.frame() %>%
  tibble::rownames_to_column("ensembl_transcript_id")
readr::write_csv(genecounts, "geo/transcript-counts-E7107.csv")
```
