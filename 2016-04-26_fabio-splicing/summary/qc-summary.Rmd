...
ihere are `r nrow(subset(prpf38, padj < 0.05))` genes differentially expressed
between the `PRPF38` and the `control` samples.

### Mean-vs-Variation
This is a diagnostic plot to make sure we aren't calling genes differentially
expressed just because we didn't model the mean-variance relationship very well.
These look good, most of our calls are within what we'd expect.
